...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(branches)} branches")
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(groups)} user groups")
            )

            # 5. Create Corporations
            corporations = self.create_corporations(users)
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(corporations)} corporations")
            )

            # 6. Create Contacts
            contacts = self.create_contacts(users, corporations)
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(contacts)} contacts")
            )

            # 7. Create Tax Cases
            cases = self.create_cases(users, contacts, corporations)
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(cases)} tax cases")
            )

            # 8. Create Case Notes
            notes = self.create_case_notes(cases, users)
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(notes)} case notes")
            )

            # 9. Create Appointments
            appointments = self.create_appointments(users, contacts, cases)
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(appointments)} appointments")
            )
            ...
            folders, documents = self.create_documents(
                users, contacts, corporations, cases
            )
            self.stdout.write(
                self.style.SUCCESS(
                    f"[OK] Created {len(folders)} folders, {len(documents)} documents"
                )
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(email_accounts)} email accounts")
            )

            # 13. Create Email Templates
            templates = self.create_email_templates(users)
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(templates)} email templates")
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"[OK] Created {len(products)} products, {len(services)} services"
                )
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(vendors)} vendors")
            )

            # 17. Create Tax Rates
            tax_rates = self.create_tax_rates()
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(tax_rates)} tax rates")
            )

            # 18. Create Invoices
            invoices = self.create_invoices(
                users, contacts, corporations, products, services, tax_rates
            )
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(invoices)} invoices")
            )

            # 19. Create Workflows
            workflows = self.create_workflows(users)
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(workflows)} workflows")
            )

            # 20. Create Notifications
            notifications = self.create_notifications(users)
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(notifications)} notifications")
            )

            # 21. Create Dashboard Widgets
            widgets = self.create_dashboard_widgets()
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(widgets)} dashboard widgets")
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"[OK] Created {len(checklists)} checklist templates"
                )
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(bh)} business hours configs")
            )

            # 24. Create Internal Tickets
            tickets = self.create_internal_tickets(users, groups)
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(tickets)} internal tickets")
            )

            # 25. Create Portal Access
            portal_access = self.create_portal_access(contacts)
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(portal_access)} portal accounts")
            )

            # 26. Create Sales Quotas and Forecasts
            quotas = self.create_sales_quotas(users)
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(quotas)} sales quotas")
            )

            # 27. Create Approvals
            approvals = self.create_approvals(users, roles)
            self.stdout.write(
                self.style.SUCCESS(f"[OK] Created {len(approvals)} approval rules")
            )
    ...
    def create_branches(self):
        ...
        branches_data = [
            ...
            {
                "name": "Coral Gables Office",
                "code": "CG",
                "address": "789 Miracle Mile",
                "city": "Coral Gables",
                "state": "FL",
                "zip_code": "33134",
                "phone": "(305) 555-0300",
    ...
    def create_users(self, roles, branches):
        ...
        users_data = [
            {
                "email": "maria.garcia@ebenezer.com",
                "first_name": "Maria",
                "last_name": "Garcia",
                "role_slug": "manager",
                "phone": "(305) 555-1001",
            ...
            {
                "email": "carlos.rodriguez@ebenezer.com",
                "first_name": "Carlos",
                "last_name": "Rodriguez",
            ...
            {
                "email": "jennifer.martinez@ebenezer.com",
                "first_name": "Jennifer",
                "last_name": "Martinez",
                "role_slug": "preparer",
                "phone": "(305) 555-1003",
            ...
            {
                "email": "david.johnson@ebenezer.com",
                "first_name": "David",
                "last_name": "Johnson",
                "role_slug": "preparer",
                "phone": "(305) 555-1004",
            ...
            {
                "email": "michael.brown@ebenezer.com",
                "first_name": "Michael",
                "last_name": "Brown",
            ...
            {
                "email": "sofia.hernandez@ebenezer.com",
                "first_name": "Sofia",
                "last_name": "Hernandez",
            ...
            {
                "email": "james.wilson@ebenezer.com",
                "first_name": "James",
                "last_name": "Wilson",
                "role_slug": "preparer",
                "phone": "(305) 555-1008",
    ...
    def create_user_groups(self, users):
        ...
        groups_data = [
            {
                "name": "Tax Preparers Team",
                "description": "All tax preparers and reviewers",
            },
            {
                "name": "Sales Team",
                "description": "Sales and business development staff",
            },
                ...
                corp = Corporation.objects.create(
                    **data,
                    ...
                    billing_po_box=(
                        ...
                        else f"PO Box {random.randint(100, 9999)}"
                    ),
                    ...
                    secondary_phone=(
                        f"(786) 555-{random.randint(2000, 9999)}"
                        if random.random() > 0.5
                        else ""
                    ),
                    fax=(
                        f"(305) 555-{random.randint(2000, 9999)}"
                        if random.random() > 0.6
                        else ""
                    ),
                    email=f"info@{data['name'].lower().replace(' ', '')}.com",
                    secondary_email=(
                        f"contact@{data['name'].lower().replace(' ', '')}.com"
                        if random.random() > 0.5
                        else ""
                    ),
    ...
    def create_contacts(self, users, corporations):
        ...
        for i in range(50):  # Create 50 contacts
            ...
            if not Contact.objects.filter(email=email).exists():
                corp = (
                    random.choice(corporations)
                    if corporations and random.random() > 0.4
                    else None
                )
                contact = Contact.objects.create(
                    ...
                    status=random.choice(
                        ["ACTIVE", "ACTIVE", "ACTIVE", "LEAD", "INACTIVE"]
                    ),  # Most active
    ...
    def create_cases(self, users, contacts, corporations):
        ...
        for i in range(40):  # Create 40 cases
            ...
            due_date = (
                date(fiscal_year + 1, 4, 15)
                if case_type == "INDIVIDUAL_1040"
                else date(fiscal_year + 1, 3, 15)
            )

            case = TaxCase.objects.create(
                case_number=f"CASE-{fiscal_year}-{case_counter:04d}",
                title=(
                    f"{case_type.replace('_', ' ').title()} - {contact.first_name} {contact.last_name}"
                    if contact
                    else f"{case_type.replace('_', ' ').title()} - {corp.name if corp else 'Client'}"
                ),
                case_type=case_type,
                ...
                actual_fee=(
                    Decimal(random.randint(200, 5000))
                    if status in ["FILED", "COMPLETED"]
                    else None
                ),
                due_date=due_date,
                filed_date=(
    ...
    def create_appointments(self, users, contacts, cases):
        ...
        for i in range(30):  # Create 30 appointments
            ...
            apt = Appointment.objects.create(
                ...
                description=(
                    f"Meeting with {contact.first_name} {contact.last_name}"
                    if contact
                    else "Client meeting"
                ),
    ...
    def create_tasks(self, users, contacts, cases, groups):
        ...
        for i in range(40):  # Create 40 tasks
            ...
            task = Task.objects.create(
                ...
                assigned_to=(
                    random.choice(users) if users and random.random() > 0.2 else None
                ),
                assigned_group=(
                    random.choice(groups) if groups and random.random() > 0.7 else None
                ),
                created_by=random.choice(users) if users else None,
                case=random.choice(cases) if cases and random.random() > 0.3 else None,
                contact=(
                    random.choice(contacts)
                    if contacts and random.random() > 0.5
                    else None
                ),
                priority=random.choice(priorities),
                status=status,
    ...
    def create_documents(self, users, contacts, corporations, cases):
        ...
        for i in range(60):  # Create 60 document records
            contact = (
                random.choice(contacts) if contacts and random.random() > 0.3 else None
            )
    ...
    def create_email_accounts(self, users):
        ...
        accounts_data = [
            {
                "name": "Main Office",
                "email_address": "office@ebenezer-tax.com",
                "imap_host": "imap.gmail.com",
                "smtp_host": "smtp.gmail.com",
            },
            {
                "name": "Support",
                "email_address": "support@ebenezer-tax.com",
                "imap_host": "imap.gmail.com",
                "smtp_host": "smtp.gmail.com",
            },
            {
                "name": "Sales",
                "email_address": "sales@ebenezer-tax.com",
                "imap_host": "imap.gmail.com",
                "smtp_host": "smtp.gmail.com",
            },
        ]
    ...
    def create_quotes(self, users, contacts, corporations, cases):
        ...
        for i in range(20):  # Create 20 quotes
            ...
            quote = Quote.objects.create(
                ...
                subject=(
                    f"Tax Services Quote - {contact.first_name} {contact.last_name}"
                    if contact
                    else "Tax Services Quote"
                ),
                stage=stage,
                valid_until=(
            ...
            for j, (svc_type, desc, price) in enumerate(
                random.sample(service_types, num_items)
            ):
                qty = Decimal("1")
                total = price * qty
                subtotal += total
        ...
        services_data = [
            {
                ...
                "service_code": "SVC-1040",
                "category": "Tax Preparation",
                "unit_price": Decimal("350.00"),
            },
            {
                "name": "Corporate Tax Return (1120)",
                "service_code": "SVC-1120",
                "category": "Tax Preparation",
                "unit_price": Decimal("1500.00"),
            },
            {
                "name": "S-Corp Tax Return (1120S)",
                "service_code": "SVC-1120S",
                "category": "Tax Preparation",
                "unit_price": Decimal("1200.00"),
            },
            {
                "name": "Partnership Return (1065)",
                "service_code": "SVC-1065",
                "category": "Tax Preparation",
                "unit_price": Decimal("1000.00"),
            },
            {
                "name": "Quarterly Bookkeeping",
                "service_code": "SVC-BOOK-Q",
                "category": "Bookkeeping",
                "unit_price": Decimal("450.00"),
            },
            {
                "name": "Monthly Bookkeeping",
                "service_code": "SVC-BOOK-M",
                "category": "Bookkeeping",
                "unit_price": Decimal("350.00"),
            },
            {
            ...
            },
            {
                ...
                "service_code": "SVC-IRS-REP",
                "category": "Representation",
                "unit_price": Decimal("500.00"),
            },
            {
                "name": "Amendment Filing",
                "service_code": "SVC-AMEND",
                "category": "Tax Preparation",
    ...
    def create_vendors(self, users):
        ...
        vendors_data = [
            ...
            {
                "name": "Tax Software Inc",
                "vendor_code": "VEND-002",
                "category": "Software",
                "email": "sales@taxsoftware.com",
                "phone": "(800) 555-2345",
            },
            {
                "name": "Print Shop Plus",
    ...
    def create_invoices(
        ...
        for i in range(25):  # Create 25 invoices
            ...
            invoice = Invoice.objects.create(
                ...
                subject=(
                    f"Tax Services - {contact.first_name} {contact.last_name}"
                    if contact
                    else "Tax Services"
                ),
                status=status,
            ...
            for j, service in enumerate(
                random.sample(services, min(num_items, len(services)))
            ):
                qty = Decimal("1")
    ...
    def create_notifications(self, users):
        ...
        notification_types = [
            (
                "CASE_STATUS_CHANGED",
                "Case Status Updated",
                "Case #CASE-2024-0015 status changed to In Progress",
            ),
            (
                "EMAIL_ASSIGNED",
                "New Email Assigned",
                "You have been assigned a new email from john.smith@email.com",
            ),
            (
                "TASK_DUE",
                "Task Due Today",
                "Task 'Call client for missing documents' is due today",
            ),
            (
                "DOCUMENT_UPLOADED",
                "New Document",
                "Client Maria Garcia uploaded a new W-2 form",
            ),
            (
                "APPOINTMENT_REMINDER",
                "Upcoming Appointment",
                "You have an appointment in 1 hour with John Smith",
            ),
        ]
        ...
        widgets_data = [
            {
                ...
                "widget_type": "CASES_BY_STATUS",
                "chart_type": "BAR",
                "description": "Distribution of cases by current status",
            },
            {
                "name": "Revenue Pipeline",
                "widget_type": "REVENUE_PIPELINE",
                "chart_type": "LINE",
                "description": "Monthly revenue projection and actuals",
            },
            {
                "name": "Upcoming Deadlines",
                "widget_type": "UPCOMING_DEADLINES",
                "chart_type": "TABLE",
                "description": "Cases with approaching due dates",
            },
            {
                "name": "Tasks Overview",
                "widget_type": "TASKS_OVERVIEW",
                "chart_type": "PIE",
            ...
            {
                "name": "Client Stats",
                "widget_type": "CLIENT_STATS",
                "chart_type": "STAT_CARD",
                "description": "Key client metrics",
            },
        ]
        ...
        templates_data = [
            {
                ...
                "items": [
                    ...
                    {
                        "title": "1099 Forms",
                        "description": "All 1099 forms (interest, dividends, etc.)",
                        "doc_type": "1099",
                        "is_required": True,
                    },
                    {
                    ...
                    },
                    {
                        ...
                        "description": "SSN card for all dependents",
                        "doc_type": "ID_DOCUMENT",
                        "is_required": True,
                    },
                    {
                        "title": "Prior Year Return",
                        "description": "Copy of last year's tax return",
                        "doc_type": "TAX_RETURN",
                        "is_required": False,
                    },
                    {
                ...
                "items": [
                    {
                        ...
                        "description": "Year-end P&L and Balance Sheet",
                        "doc_type": "OTHER",
                        "is_required": True,
                    },
                    {
                        "title": "Bank Statements",
                        "description": "All business bank accounts",
                        "doc_type": "BANK_STATEMENT",
                        "is_required": True,
                    },
                    {
                        "title": "Payroll Reports",
                        "description": "Annual payroll summary",
                        "doc_type": "OTHER",
                        "is_required": True,
                    },
                    {
                        "title": "Depreciation Schedule",
                        "description": "Asset depreciation records",
                        "doc_type": "OTHER",
                        "is_required": False,
                    },
                    {
                        "title": "Prior Year Return",
                        "description": "Previous corporate return",
                        "doc_type": "TAX_RETURN",
                        "is_required": True,
                    },
                ],
        ...
        tickets_data = [
            {
                ...
                "description": "My computer has been running very slow since last week.",
                "category": "IT",
                "priority": "NORMAL",
            },
            {
            ...
            },
            {
                ...
                "description": "Question about my latest pay stub.",
                "category": "HR",
                "priority": "NORMAL",
            },
            {
                "title": "Office supplies needed",
                "description": "Need printer paper and pens for the front desk.",
                "category": "GENERAL",
                "priority": "LOW",
            },
            {
                "title": "Software license renewal",
                "description": "Tax software license expiring next month.",
                "category": "IT",
                "priority": "HIGH",
            },
            {
                "title": "AC not working",
                "description": "The air conditioning in the main office is not working.",
                "category": "GENERAL",
                "priority": "URGENT",
            },
        ]
