...
"""
    ...
    def analyze_business_metrics(self) -> list[AgentInsight]:
        ...
        try:
            ...
            self._log("debug", "Metrics gathered", {"metrics_summary": self._summarize_metrics(metrics)})
    ...
    def _get_case_metrics(self) -> dict[str, Any]:
        ...
        by_status = dict(
            TaxCase.objects.values("status").annotate(count=Count("id")).values_list("status", "count")
        )

        by_type = dict(
            TaxCase.objects.values("case_type").annotate(count=Count("id")).values_list("case_type", "count")
        )
    ...
    def _get_client_metrics(self) -> dict[str, Any]:
        ...
        contacts_with_active_cases = Contact.objects.filter(
            tax_cases__status__in=["intake", "document_collection", "in_preparation", "in_review"],
        ).distinct().count()
        ...
        return {
            ...
            "engagement_rate": round(contacts_with_active_cases / total_contacts * 100, 1) if total_contacts > 0 else 0,
        }
    ...
    def _get_task_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "overdue_percentage": round(overdue / total_open * 100, 1) if total_open > 0 else 0,
    ...
    def _get_appointment_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "no_show_rate": round(no_shows / (completed + no_shows) * 100, 1) if (completed + no_shows) > 0 else 0,
    ...
    def _summarize_metrics(self, metrics: dict) -> dict:
        ...
        return {
            ...
            "total_contacts": metrics.get("clients", {}).get("total_active_contacts", 0),
            "overdue_tasks": metrics.get("tasks", {}).get("overdue_tasks", 0),
            "upcoming_appointments": metrics.get("appointments", {}).get("upcoming_next_week", 0),
        }
    ...
    def _generate_ai_insights(self, metrics: dict) -> list[AgentInsight]:
        ...
        focus_areas = self.config.focus_areas or ["revenue", "efficiency", "client_satisfaction"]
...
Generate 3-5 most important insights."""
    ...
    @transaction.atomic
    def _save_insights(self, insights_data: list, ai_result: dict | None) -> list[AgentInsight]:
        ...
        self._log(
            ...
            {"action_id": str(action.id), "insight_ids": [str(i.id) for i in saved_insights]},
            action=action,
        )
